...
def aggregate_statistics(counts, means, devs):
    ...
    s = 0
    for i in r:
        s += counts[i] * (means[i] - mean) ** 2 + (counts[i]-1)*(devs[i]**2)
    dev = np.sqrt(1 / (n-1) * s)

    return mean, dev

def run_test_with_saving(gen, n, saves, filename, verbose=True, i=859000000):
    run = n//saves
    ret = []
    counts = []
    means=[]
